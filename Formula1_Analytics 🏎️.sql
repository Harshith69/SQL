...
Select raceid, round(avg(fastestLapSpeed)) as Topspeed from f1 where fastestLapSpeed !=0 group by raceid order by Topspeed desc;

-- Identify the drivers with the most points (highest position order)
Select drivername from f1 group by drivername order by sum(points) desc limit(1);
